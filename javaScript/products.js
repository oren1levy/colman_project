...
//                     <figcaption class="productprice" data-productPrice="productprice"><del id="deletedprice">${product.price}₪</del>&nbsp;${discount}₪</figcaption>
//                     <div class="productColor">
//                         ${Array.isArray(product.color) ? product.color.map(color => `<span style="background-color: ${color};" class="color-span"></span>`).join('') : ''}
//                     </div>
//                 </figure>
//             `;
//             productGallery.insertAdjacentHTML('beforeend', productElement);
...
//         const categories = Array.from(document.querySelectorAll('input[name="jewelryType"]:checked')).map(cb => cb.value);
...
//         document.querySelectorAll('input[name="jewelryType"]').forEach(cb => cb.checked = false);
...
//         document.getElementById('priceRangeValue').textContent = `0 - ${price}₪`;
//     });
